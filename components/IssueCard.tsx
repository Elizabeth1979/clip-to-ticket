      ...
      <div className="p-6 border-b border-slate-50 flex justify-between items-start gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3">
            <button
              onClick={onSeek}
              className="text-sm tracking-widest text-slate-400 hover:text-indigo-600 flex items-center gap-1.5 transition-colors"
            >
            ...
            <div className="flex items-center gap-1.5">
              <span className={`text-sm px-2 py-0.5 rounded-md border tracking-widest ${getSeverityStyles(issue.severity)}`}>
                {issue.severity}
              </span>
        ...
        <div className="text-right flex items-center gap-1.5">
          {isManual ? (
            <span className="text-sm tracking-widest text-slate-400 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
              Manual Audit
            </span>
          ) : (
            <a
              ...
              className="text-sm tracking-widest text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 hover:bg-indigo-100 transition-colors underline"
            >
              {issue.axe_rule_id}
            </a>
          )}
        ...
        <div className="space-y-3">
          ...
          <div className="pt-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-900 transition-colors underline"
            >
              <span className="tracking-widest text-sm text-slate-300">Reference:</span> {issue.wcag_reference}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {issue.generated_alt_text && (
            <div className="p-4 bg-emerald-50/50 rounded-xl border border-emerald-100 flex flex-col justify-center">
              <h4 className="text-sm text-emerald-600 tracking-widest mb-1.5">Proposed Alt Text</h4>
          ...
          <div className={`p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-center ${!issue.generated_alt_text ? 'md:col-span-2' : ''}`}>
            <h4 className="text-sm text-slate-500 tracking-widest mb-1.5">Actionable Fix</h4>
